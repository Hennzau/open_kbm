...
fn key_event(event_source: CGEventSource, key: u16, state: u8, modifiers: XMods) {
    let event = match CGEvent::new_keyboard_event(event_source, key, state != 0) {
        Ok(e) => e,
        Err(_) => {
            println!("unable to create key event");
            return;
        }
    };
...
fn modifier_event(event_source: CGEventSource, depressed: XMods) {
    let Ok(event) = CGEvent::new(event_source) else {
        println!("could not create CGEvent");
        return;
    };
    ...
    event.post(CGEventTapLocation::HID);
    println!("modifiers updated: {depressed:?}");
}
...
fn get_display_at_point(x: CGFloat, y: CGFloat) -> Option<CGDirectDisplayID> {
    ...
    if error != 0 {
        println!("error getting displays at point ({}, {}): {}", x, y, error);
        return Option::None;
    }

    if display_count == 0 {
        println!("no displays found at point ({}, {})", x, y);
        return Option::None;
    }
...
fn clamp_to_screen_space(
    ...
    let current_display = match get_display_at_point(current_x, current_y) {
        ...
        None => {
            println!("could not get current display!");
            ...
            Event::Pointer(pointer_event) => match pointer_event {
                PointerEvent::Motion { time: _, dx, dy } => {
                    let mut mouse_location = match self.get_mouse_location() {
                        Some(l) => l,
                        None => {
                            println!("could not get mouse location!");
                            return Ok(());
                        }
                    };
                    ...
                    let event = match CGEvent::new_mouse_event(
                        ...
                        Ok(e) => e,
                        Err(_) => {
                            println!("mouse event creation failed!");
                            return Ok(());
                        }
                    };
                ...
                PointerEvent::Button {
                    ...
                    let (event_type, mouse_button) = match (button, state) {
                        ...
                        _ => {
                            println!("invalid button event: {button},{state}");
                            return Ok(());
                        }
                    };
                    // store button state
                ...
                PointerEvent::Axis {
                    ...
                    let (count, wheel1, wheel2, wheel3) = match axis {
                        ...
                        _ => {
                            println!("invalid scroll event: {axis}, {value}");
                            return Ok(());
                        }
                    };
                    let event = match CGEvent::new_scroll_event(
                        ...
                        Ok(e) => e,
                        Err(()) => {
                            println!("scroll event creation failed!");
                            return Ok(());
                        }
                    };
                    event.post(CGEventTapLocation::HID);
            ...
            Event::Keyboard(keyboard_event) => match keyboard_event {
                KeyboardEvent::Key {
                    ...
                    let code = match KeyMap::from_key_mapping(KeyMapping::Evdev(key as u16)) {
                        ...
                        Err(_) => {
                            println!("unable to map key event");
                            return Ok(());
                        }
                    };
                    match state {
