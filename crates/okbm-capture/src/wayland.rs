...
impl Window {
    fn new(
        ...
        println!("creating window output: {output:?}, size: {size:?}");
        let g = &state.globals;
...
impl LayerShellInputCapture {
    pub fn new() -> Result<Self> {
        ...
        let compositor: wl_compositor::WlCompositor = global_list.bind(&qh, 4..=5, ())?;
        let xdg_output_manager: ZxdgOutputManagerV1 = global_list.bind(&qh, 1..=3, ())?;
        let shm: wl_shm::WlShm = global_list.bind(&qh, 1..=1, ())?;
        let layer_shell: ZwlrLayerShellV1 = global_list.bind(&qh, 3..=4, ())?;
        let seat: wl_seat::WlSeat = global_list.bind(&qh, 7..=8, ())?;
        ...
        let shortcut_inhibit_manager = global_list.bind(&qh, 1..=1, ());
...
impl State {
    ...
    fn register_global(&mut self, global: Global) {
        if global.interface.as_str() == "wl_output" {
            println!("new output global: wl_output {}", global.name);
    ...
    fn deregister_global(&mut self, name: u32) {
        self.outputs.retain(|o| {
            if o.global.name == name {
                println!("{o} (global {:?}) removed", o.global);
                o.wl_output.release();
                false
            } else {
                true
    ...
    fn add_client(&mut self, pos: Position) {
        ...
        println!(
            "adding capture for position {:?} - using outputs: {:?}",
            pos,
            outputs
                .iter()
                .map(|o| o
        ...
        match event {
            ...
            wl_pointer::Event::Leave { .. } => {
                ...
                if app.pointer_lock.is_some() {
                    println!("compositor released mouse");
                }
                app.ungrab();
            }
    ...
    fn event(
        ...
        match event {
            ...
            zxdg_output_v1::Event::Done => {
                println!("Use of deprecated xdg-output event \"done\"");
